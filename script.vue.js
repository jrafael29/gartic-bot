(function () {
  ...
  loadVue(() => {
    ...
    if (document.getElementById(boxId)) {
      console.warn("[Gartic Vue] Já existe uma instância ativa.");
      return;
    }
    ...
    const App = {
      ...
      setup() {
        ...
        const selectSuggestion = (item) => {
          search.value = item; // preenche o input com a sugestão
        };
        ...
        const suggestionInputPlaceholder = computed(() => {
          return !window.esperandoNovaPartida ? "Buscar sugestões..." : "Você acertou!"
        })
              ...
              muts.forEach((m) => {
                m.addedNodes.forEach((node) => {
                  ...
                  if (
                    ...
                    console.log("[Gartic Bot] Você acertou o desenho");
                    window.acertou = true;
                    window.esperandoNovaPartida = true;
                    acertou.value = true;
        ...
        ">
          ...
          <div v-show="showMessages" style="display:flex; justify-content: space-between; align-items: center;">
            <div style="display:flex; justify-content: center; align-items: center; gap: 5px; font-weight:bold; text-align:center; margin-bottom:5px;">
             <i width="16" :data-lucide="messageSquareTextIcon"></i> Histórico do Chat
            </div>
  ...
  const extractMessageData = (div) => {
    ...
    if (nickname) {
      // valida se o name da msg é o name do usuario atual
  ...
  window.allahuAkbar = () => {
    ...
    if (oldContainer) {
      oldContainer.remove(); // remove a instância antiga
    }
  };
})();
